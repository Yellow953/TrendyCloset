    ...
    <div class="bo-card">
        <form method="GET" class="flex flex-wrap items-end gap-3 border-b border-slate-100 px-5 py-4">
            <div class="min-w-[200px] flex-1">
                ...
                <input id="q" name="q" value="{{ request('q') }}" placeholder="Order number, phone or name…" class="bo-input">
            </div>
            ...
            <div class="w-[150px]">
                <label for="range" class="bo-label">Placed</label>
                <select id="range" name="range" class="bo-input">
            ...
            @if(request()->hasAny(['q', 'status', 'range']))
                <a href="{{ route('admin.orders.index') }}" class="bo-btn">Clear</a>
            @endif
        </form>
        ...
        @elseif($orders->isEmpty())
            <x-admin.empty icon="orders" title="No orders yet"
                ...
                <a href="{{ route('home') }}" target="_blank" rel="noopener" class="bo-btn">View the shop ↗</a>
            </x-admin.empty>
        @else
                    ...
                    <tbody>
                        @foreach($orders as $order)
                            <tr>
                                ...
                                <td class="bo-figure text-right font-medium whitespace-nowrap">{{ \App\Models\Product::money($order->grand_total) }}</td>
                                <td class="text-right">
                                    <a href="{{ route('admin.orders.show', $order) }}" class="bo-btn bo-btn-sm">Open</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    @foreach($orders as $order)
        <x-admin.modal :id="'status-'.$order->id"
            ...
            <form method="POST" action="{{ route('admin.orders.status', $order) }}">
                ...
                <div class="px-6 py-5">
                    <span class="bo-label">Move this order to</span>

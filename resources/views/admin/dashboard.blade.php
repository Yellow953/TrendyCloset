...
@section('actions')
    ...
    @if(auth()->user()->isAdmin())
        <a href="{{ route('admin.analytics') }}" class="bo-btn">Analytics</a>
    @endif
@endsection
...
@section('content')
        ...
        @foreach($kpis as $kpi)
            <div class="bo-card p-5">
                ...
                <div class="mt-2.5 flex flex-wrap items-baseline gap-x-2.5 gap-y-1.5">
                    <span class="bo-figure text-[24px] leading-none font-medium">{{ $kpi['value'] }}</span>
                    @if($kpi['delta'] !== null)
    ...
    <div class="bo-card mt-5">
        <div class="bo-card-head">
            <div class="bo-card-title">Views, bags and orders</div>
        ...
        <div class="flex flex-wrap gap-x-10 gap-y-3 border-t border-slate-100 px-5 py-3.5">
            @foreach($funnel['steps'] as $step)
                <div class="flex items-baseline gap-2">
                    ...
                    <span class="bo-figure text-[13.5px] font-semibold text-slate-800">
                        {{ $step['rate'] !== null ? $step['rate'].'%' : '—' }}
                    </span>
                    <span class="bo-figure text-[11px] font-normal text-slate-400">{{ $step['hint'] }}</span>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-3">
        ...
        <div class="bo-card">
            <div class="bo-card-head">
                <div class="bo-card-title">Latest orders</div>
                    ...
                    @foreach($recentOrders as $order)
                        <a href="{{ route('admin.orders.show', $order) }}"
                           class="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-slate-50">
                            <div class="min-w-0">
                                <div class="bo-figure truncate text-[12.5px] font-medium">{{ $order->order_number }}</div>
                            ...
                            <div class="flex shrink-0 items-center gap-2.5">
                                ...
                                <span class="bo-figure text-[13px] font-medium">{{ \App\Models\Product::money($order->grand_total) }}</span>
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="bo-card">
            <div class="bo-card-head">
                <div class="bo-card-title">Running out</div>
                <span class="bo-badge bo-badge-warn">≤ {{ \App\Http\Controllers\Admin\DashboardController::LOW_STOCK }} left</span>
            </div>
                    ...
                    @foreach($lowStock as $variant)
                        <a href="{{ route('admin.products.edit', $variant->product) }}"
                           class="flex items-center justify-between gap-3 px-5 py-3 transition-colors hover:bg-slate-50">
                            ...
                            <span class="bo-badge shrink-0 {{ $variant->stock === 0 ? 'bo-badge-bad' : 'bo-badge-warn' }}">
                                <span class="bo-figure">{{ $variant->stock }}</span> left
                            </span>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="bo-card">
            <div class="bo-card-head">
                <div class="bo-card-title">Unread enquiries</div>

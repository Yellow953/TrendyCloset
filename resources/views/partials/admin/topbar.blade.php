    ...
    <div class="mx-auto flex w-full max-w-[1400px] items-center gap-4 px-5 py-3 md:px-8">
        ...
        <form method="GET" action="{{ route('admin.products.index') }}" class="relative hidden max-w-[400px] flex-1 sm:block">
            ...
            <input type="search" name="q" value="{{ request('q') }}" placeholder="Search products…"
                   class="bo-input rounded-lg py-2 pl-10 text-[13px]">
        </form>

        <div class="ml-auto flex items-center gap-2">
            <a href="{{ route('admin.orders.index', ['status' => 'pending']) }}"
               class="bo-btn bo-btn-sm hidden sm:inline-flex">
                <x-admin.icon name="bell" class="h-4 w-4 text-slate-400" />
                <span class="bo-figure">{{ $openOrders ?? 0 }}</span> to fulfil
            </a>
            <a href="{{ route('home') }}" target="_blank" rel="noopener" class="bo-btn bo-btn-sm">
            ...
            <details class="bo-menu relative ml-1" data-admin-menu>
                ...
                <div class="bo-card absolute right-0 z-30 mt-2 w-60 overflow-hidden">
                    <div class="border-b border-slate-100 px-4 py-3.5">
                        ...
                        <span class="bo-badge bo-badge-neutral mt-2.5">{{ auth()->user()->role->label() }}</span>
                    </div>
